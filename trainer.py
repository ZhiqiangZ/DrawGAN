    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                ######################################################
                data = data_iter.next()
                imgs, seg_imgs, fore_imgs, captions, cap_lens, class_ids, keys = prepare_data(data)
                ...
                for i in range(len(netsD)):
                    if i == 0:
                        netsD[i].zero_grad()
                        errD, log = discriminator_loss(netsD[i], seg_imgs[i], fake_imgs[i],
                    ...
                    if i == 1:
                        netsD[i].zero_grad()
                        errD, log = discriminator_loss(netsD[i], fore_imgs[i], fake_imgs[i],
                                                  sent_emb, real_labels, fake_labels)
                        # backward and update parameters
                        errD.backward()
                        optimizersD[i].step()
